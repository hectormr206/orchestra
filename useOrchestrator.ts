...
export function useOrchestrator(): [OrchestratorState, OrchestratorActions] {
  ...
  const start = useCallback(async (task: string, options: {
    ...
    durationIntervalRef.current = setInterval(() => {
      ...
      const roundedDuration = Math.round(currentDuration / 100) * 100;

      if (roundedDuration !== lastDurationRef.current) {
        lastDurationRef.current = roundedDuration;
        setState((prev) => ({
          ...prev,
  ...
  const cancel = useCallback(() => {
    ...
    if (planResolveRef.current) {
      planResolveRef.current({ approved: false, reason: 'cancelled' });
      planResolveRef.current = null;
    }
    ...
    setState((prev) => {
      const shouldLogCancel = prev.phase !== 'complete' && prev.phase !== 'error';
      if (shouldLogCancel) {
