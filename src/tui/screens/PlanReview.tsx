...
export const PlanReview: React.FC<PlanReviewProps> = ({
  ...
  useInput((input, key) => {
    ...
    if (key.downArrow) {
      setScrollOffset((prev) => Math.min(planLines.length - visibleLines, prev + 1));
    }
    if (key.return) {
  ...
  const visiblePlan = planLines.slice(scrollOffset, scrollOffset + visibleLines);
  const scrollPercent = planLines.length > visibleLines
  ...
  return (
    <Box flexDirection="column" padding={1}>
      ...
      <Box marginTop={2} borderStyle="single" borderColor="gray" paddingX={1}>
        <Text color="gray">
          ←/→: Select action │ ↑/↓: Scroll │ Enter: Confirm │ a/r/e: Quick select
        </Text>
      </Box>
    </Box>
  );
