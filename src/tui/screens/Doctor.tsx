...
export const Doctor: React.FC<DoctorProps> = ({ onBack }) => {
  ...
  useEffect(() => {
    const runChecks = async () => {
      ...
      try {
        ...
        updateCheck('Codex CLI', { status: 'warning', message: 'Optional - for Consultant' });
      }
      ...
      try {
        ...
        updateCheck('Gemini CLI', { status: 'warning', message: 'Optional - for fallback' });
      }
      ...
      try {
        ...
        updateCheck('Python 3', { status: 'error', message: 'Required for validation' });
      }
      ...
      if (process.env.ZAI_API_KEY) {
        ...
        updateCheck('ZAI_API_KEY', { status: 'error', message: 'Not configured' });
      }

      // GitHub CLI
      try {
        const { stdout } = await execFileAsync('gh', ['--version']);
        updateCheck('GitHub CLI', { status: 'ok', version: stdout.split('\n')[0] });
      } catch {
        updateCheck('GitHub CLI', { status: 'warning', message: 'Optional - for GitHub integration' });
      }
      ...
      if (foundConfig) {
        ...
        updateCheck('Config File', { status: 'warning', message: 'Using defaults' });
      }
    };
  ...
  const getStatusIcon = (status: CheckResult['status']) => {
    switch (status) {
      case 'checking': return <Spinner type="dots" />;
      case 'ok': return <Text color="green">✓</Text>;
      case 'warning': return <Text color="yellow">⚠</Text>;
  ...
  return (
    <Box flexDirection="column" padding={1}>
      <Box borderStyle="double" borderColor="cyan" paddingX={2}>
        <Text bold color="cyan">🩺 SYSTEM DOCTOR</Text>
      </Box>

      <Box flexDirection="column" marginTop={1} borderStyle="single" borderColor="gray" padding={1}>
        {checks.map((check) => (
          <Box key={check.name}>
            <Box width={4}>{getStatusIcon(check.status)}</Box>
            <Box width={15}>
              <Text color="white">{check.name}</Text>
            </Box>
            <Box>
              {check.version && (
                <Text color={getStatusColor(check.status)}>{check.version}</Text>
              )}
              {check.message && (
                <Text color={getStatusColor(check.status)}> {check.message}</Text>
              )}
            </Box>
          </Box>
        ))}
